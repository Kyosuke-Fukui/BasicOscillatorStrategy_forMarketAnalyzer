//損益計算
var PL = function (xArr, yArr, p1, pLow, pHigh, risk_rng, reward, dma, TrStop) {
  var sigarr = getDataSet(yArr, p1, pLow, pHigh, dma)[2];
  var PL_tan = 0; //損益率
  ...
  for (let i = 0; i < yArr.length; i++) {
    //買いシグナル点灯かつ買いポジション未保有時
    if (sigarr[i] === 1 && posiflg !== 1) {
      //売りポジションがあれば決済
      if (posiflg == -1) {
      ...
      //売りシグナル点灯かつ売りポジション未保有時
    } else if (sigarr[i] === -1 && posiflg !== -1) {
      //買いポジションがあれば決済
      if (posiflg == 1) {
...
async function plot_PL(xArr, yArr) {
  ...
  var pLow = parseInt($("#pLow").val());
  var pHigh = parseInt($("#pHigh").val());
  var risk = $("#risk").val() / 100;
  var reward = $("#reward").val() / 100;
  var dma = parseInt($("#DMA").val());
  var TrStop = $("#TrStop").val() / 100;
  var TR = PL(xArr, yArr, p1, pLow, pHigh, risk, reward, dma, TrStop)[0];
  var PL_tan = PL(
    xArr,
    yArr,
    p1,
  ...
  var buyArr = PL(xArr, yArr, p1, pLow, pHigh, risk, reward, dma, TrStop)[5];
  var sellArr = PL(xArr, yArr, p1, pLow, pHigh, risk, reward, dma, TrStop)[6];
  var slArr = PL(xArr, yArr, p1, pLow, pHigh, risk, reward, dma, TrStop)[7];
  var tpArr = PL(xArr, yArr, p1, pLow, pHigh, risk, reward, dma, TrStop)[8];
...
async function getGraph_S(xArr, yArr) {
  ...
  var dataArrs = getDataSet(yArr, p1, pLow, pHigh, dma);

  var d1 = dataArrs[0]; //原データ
  var d2 = dataArrs[1]; //RCI
  // var d3 = dataArrs[2];
  var n1 = dataname;
  var n2 = `RSI(${p1})`;
